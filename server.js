...
app.post('/api/generate-pdf', (req, res) => {
  try {
    ...
    doc.fontSize(16)
       .font('Helvetica-Bold')
       .text('QUOTATION', 40, 135);

    // Quotation details
    doc.fontSize(9)
       .font('Helvetica');
    
    const quoteDate = new Date(quotationData.quotationDate).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    ...
    doc.text(`Quotation ID: QT-${quotationData.id || Math.floor(Math.random() * 100000)}`, 40, 175);

    // Client details
    doc.fontSize(11)
       .font('Helvetica-Bold')
       .text('CLIENT INFORMATION', 40, 205);
    ...
    doc.fillColor('#ffffff')
       ...
       .text('Qty', 250, tableTop + 5, { width: colWidths.quantity, align: 'right' })
       .text('Unit Price', 310, tableTop + 5, { width: colWidths.unitPrice, align: 'right' })
       .text('Total', 450, tableTop + 5, { width: colWidths.total, align: 'right' });
    ...
    quotationData.items.forEach((item, index) => {
      ...
      doc.fontSize(9)
         .text(item.name + ' - ' + item.description, 50, currentY, { width: 190 })
         .text(item.quantity, 250, currentY, { width: colWidths.quantity, align: 'right' })
         .text(formatCurrency(item.unitPrice), 310, currentY, { width: colWidths.unitPrice, align: 'right' })
         .text(formatCurrency(itemTotal), 450, currentY, { width: colWidths.total, align: 'right' });
    ...
    const discountAmount = quotationData.discount ? 
      (subtotal * parseFloat(quotationData.discount) / 100) : 0;
    
    const subtotalAfterDiscount = subtotal - discountAmount;
    const taxAmount = subtotalAfterDiscount * (parseFloat(quotationData.tax) / 100);
    ...
    doc.text('Subtotal:', totalsX, currentY, { width: 100 })
       .text(formatCurrency(subtotal), totalsRightX, currentY, { width: 60, align: 'right' });
    
    currentY += 20;
    
    if (quotationData.discount && parseFloat(quotationData.discount) > 0) {
      doc.text(`Discount (${quotationData.discount}%):`, totalsX, currentY, { width: 100 })
         .text('-' + formatCurrency(discountAmount), totalsRightX, currentY, { width: 60, align: 'right' });
      currentY += 20;
    }

    doc.text(`Tax (${quotationData.tax}%):`, totalsX, currentY, { width: 100 })
       .text(formatCurrency(taxAmount), totalsRightX, currentY, { width: 60, align: 'right' });
    ...
    doc.fillColor('#ffffff')
       ...
       .text(formatCurrency(grandTotal), totalsRightX, currentY, { width: 60, align: 'right' });
    ...
    doc.fillColor('#666666')
       .fontSize(8)
       .font('Helvetica')
       .text('Professional Solutions for Your Business', 40, currentY + 15, { align: 'center' })
       .text('This quotation is valid for 30 days from the date of issue.', 40, currentY + 28, { align: 'center' })
